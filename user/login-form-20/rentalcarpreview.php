...
<body>
    ...
    <div class="img js-fullheight" style="background-image: url(images/review.jpg); ">
        ...
        if (isset($_POST['submit'])) {
            ...
            $q = mysqli_query($con, "insert into rental_car_booking values('',$user_id,'$user_name',$user_contact,'$user_email',$carmodel_id,$cardetail_id,'$liecense_no','$booking_date','$pick_date','$return_date','$pick_time',$price_day,$total_day,$total_amount,0)");

            echo "insert into rental_car_booking values('',$user_id,'$user_name',$user_contact,'$user_email',$carmodel_id,$cardetail_id,'$liecense_no','$booking_date','$pick_date','$return_date','$pick_time',$price_day,$total_day,$total_amount,0)";
...
Thank you for joining BOOK YOUR RIDE ,We are delighted to inform you , Your car is all Santized and cleaned up to drive along with you on date: $pick_date time: $pick_time Days: $total_days.You can pick up your Ride from Book your ride office at 7,square arcade , Adajan ,Surat.Hope you enjoy the ride and our services . Please add your review from our website and let us know how was your experience. The amount you need to pay for the ride is $total_amount.
    ...
    Team Book your Ride";
            ...
            if ($q) {
                ...
                $q1 = mysqli_query($con, "update rental_car_booking set return_status=0 where carmodel_id=$carmodel_id");
                $q2 = mysqli_query($con, "update cardetail set 	status=1 where cardetail_id=$cardetail_id");
                $q3 = mysqli_query($con, "update carmodel set status=1 where carmodel_id=$carmodel_id ");
                // if (($q) && ($q1) && ($q2)) {
                if (mail($to, $subject, $message, $headers) && ($q1) && ($q2) && ($q3)) {

                    // echo "<script>alert('Inserted......');</script>";
                    echo "<script>window.location.assign('../paymentinvoice/carpayment_invoice.php?carmodel_id=$carmodel_id&cardetail_id=$cardetail_id')</script>";
                } else {
                    echo "<script>alert('Not Update.....plz try again....');</script>";
                }
            } else {
                            ...
                            <form action="" class="request-form ftco-animate bg-primary" style=" background: #0f172b !important; width: 100%;" method="POST" enctype="multipart/form-data">
                                ...
                                <!-- <div class="d-flex">
                                    <div class="form-group ml-2">
                                        ...
                                        <div class="col-sm-2">
                                            <image src='images/<?php echo $row['bikebrand_photo']; ?>' height=100 width=100>
                                        </div>
                                    </div>
                                    <div class="form-group ml-2">
                                ...
                                <div class="form-group ml-2">
                                    <label for="" class="label">Liecense Front Photo</label>
                                    <input type="hide" class="form-control" id="front_photo" name="front_photo" placeholder="eg. Mumbai">
                                </div>
                                <div class="form-group ml-2">
                                    <label for="" class="label">Liecense Rear Photo</label>
                                    <input type="hide" class="form-control" id="back_photo" name="back_photo" placeholder="eg. Mumbai">
                                </div> -->
                            <!-- </div> -->
                                <div class="d-flex">
                                    <div class="form-group mr-2">
                                        <label for="" class="label">Pick-up Date</label>
                                        <input type="date" class="form-control" style="background-color: #22293c; width: 250px" id="pick_date" name="pick_date" value="<?php echo $pick_date; ?>" placeholder="Date" readonly>
                                    </div>
                                    <div class="form-group ml-2">
                                        <label for="" class="label">Return Date</label>
                                        <input type="date" class="form-control" style="background-color: #22293c; width: 250px;" id="return_date" name="return_date" value="<?php echo $return_date; ?>" placeholder="Date" onchange="chk()" readonly>
                                    </div>
                                </div>
                                <div class="form-group ml-2">
